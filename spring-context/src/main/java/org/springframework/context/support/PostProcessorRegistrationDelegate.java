 ...
 * AbstractApplicationContext的委托后处理器处理。
...
final class PostProcessorRegistrationDelegate {
	...
	/**
	 * 调用实现了 BeanFactoryPostProcessor 的所有后置处理器
	 * @param beanFactory configurableListableBeanFactory 类型的 DefaultListableBeanFactory对象
	 * @param beanFactoryPostProcessors  BeanFactory的后置处理器集合
	 */
	public static void invokeBeanFactoryPostProcessors(
		...
		if (beanFactory instanceof BeanDefinitionRegistry) {
			...
			// 未实现 BeanDefinitionRegistryPostProcessor 接口的 BeanFactoryPostProcessor 实现
			...
			for (String ppName : postProcessorNames) {
				//判断是否实现了排序接口 PriorityOrdered，并实例化
			...
			//调用postProcessBeanFactory方法,在应用程序上下文的标准初始化之后  修改它的内部bean工厂。
			// 所有bean定义都已加载，但还没有实例化任何bean。这允许覆盖或添加属性，甚至对急于初始化的bean也是如此。
	...
	/**
	 * 实例化并调用所有已注册的BeanPostProcessor bean，如果给定，则遵循显式顺序。必须在应用程序bean的任何实例化之前调用。
	 * @param beanFactory
