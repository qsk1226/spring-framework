...
public class ReflectiveAspectJAdvisorFactory extends AbstractAspectJAdvisorFactory implements Serializable {
	...
	public List<Advisor> getAdvisors(MetadataAwareAspectInstanceFactory aspectInstanceFactory) {
		...
		for (Field field : aspectClass.getDeclaredFields()) {
			// 判断属性上时候有DeclareParents注解，如果有返回切面
	...
	public Advisor getAdvisor(Method candidateAdviceMethod, MetadataAwareAspectInstanceFactory aspectInstanceFactory,
		...
		// 创建Advisor切面类，这才是真正的切面类，一个切面类里面肯定有1、pointCut 2、advice
	...
	public Advice getAdvice(Method candidateAdviceMethod, AspectJExpressionPointcut expressionPointcut,
		...
		// 找到candidateAdviceMethod 方法上面的注解，并且包装成AspectJAnnotation对象，这个对象中没有注解类型
		AspectJAnnotation<?> aspectJAnnotation =
				AbstractAspectJAdvisorFactory.findAspectJAnnotationOnMethod(candidateAdviceMethod);
		if (aspectJAnnotation == null) {
			return null;
		...
		switch (aspectJAnnotation.getAnnotationType()) {
			...
			case AtAround:
				...
				break;
			case AtBefore:
				// 实现了 MethodBeforAdvice 接口，没有实现MethodInterceptor接口
